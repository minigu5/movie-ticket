    ...
    try {
      const res = await fetch('/api/auth/request-reset', {
        ...
        body: JSON.stringify({
          ...
          baseUrl: window.location.origin,
          returnUrl: `/cancel?ticketId=${ticketId}` // 🌟 재설정 후 다시 여기로 돌아오도록 주소 전달!
        })
      });
      if (res.ok) {
    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
      <div className="bg-gray-800 p-8 rounded-xl max-w-sm w-full text-center border border-gray-700 shadow-2xl">
        ...
        <input
          ...
          className="w-full p-3 rounded-lg bg-gray-700 text-white border border-gray-600 mb-4 text-center text-xl tracking-widest"
        />
        
        <button onClick={handleCancel} className="w-full py-3 bg-red-600 hover:bg-red-500 rounded-lg text-white font-bold transition-colors mb-4">
          예매 취소하기
        </button>

        {showResetButton && (
          <button onClick={handleRequestReset} disabled={isResetting} className="text-sm text-yellow-400 hover:text-yellow-300 underline underline-offset-4 transition-colors font-bold">
