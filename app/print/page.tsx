...
export default function KioskPrintPage() {
  ...
  const[ticketData, setTicketData] = useState<any>(null); // 출력할 영수증 데이터
  const[isPrinting, setIsPrinting] = useState(false);
  
  const [showResetButton, setShowResetButton] = useState(false);
  const [isResetting, setIsResetting] = useState(false);

  // 1. 초기 렌더링 시 영화 정보 불러오기
  useEffect(() => {
  ...
  // 2. 티켓 데이터가 세팅되면 브라우저 인쇄창을 띄우기 (렌더링 대기 시간 약간 줌)
  useEffect(() => {
    if (ticketData) {
  ...
  // 3. 인쇄 창이 닫히면 (확인을 누르든 취소를 누르든) 바로 초기 화면으로 복귀
  useEffect(() => {
  ...
  const handleRequestReset = async () => {
    ...
    try {
      const res = await fetch('/api/auth/request-reset', {
        method: 'POST',
        // 학생이 폰에서 확인하므로 returnUrl을 print 페이지로 설정
  ...
  const handlePrintSubmit = async () => {
    ...
    try {
      ...
      // 5. 화면에 영수증을 렌더링하도록 상태 업데이트 (useEffect가 렌더링 후 자동 인쇄 실행)
  ...
  return (
    <>
      ...
      {/* 전체 컨테이너: 화면에서는 다크모드, 프린트 시에는 하얀색 배경 */}
      <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center p-4 print:bg-white print:text-black print:min-h-0 print:p-0 print:block select-none">
        ...
        {!ticketData ? (
          // 💻 [화면용 UI] 티켓 정보 입력 화면 (프린트 시에는 숨김 처리)
          ...
          // 🧾[프린트용 UI] 감열식 프린터 전용 영수증 (화면에서는 숨기고 프린트 모드에서만 나타남)
          // 영수증 프린터의 일반적인 너비인 80mm로 강제 지정하여 인쇄 품질 보장
          <div className="w-[80mm] mx-auto bg-white text-black font-mono print:w-full print:m-0">
            
            <div className="text-center text-2xl font-black mb-1 tracking-widest">영화대교 입장권</div>
            ...
            {/* 흑백 프린터에서도 그럴싸하게 나오는 가짜 바코드 디자인 */}
