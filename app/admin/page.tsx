...
export default function AdminPage() {
  ...
  const handleSendPromo = async () => {
    ...
    // 1. 테스트 계정 추가 (영화대교 동아리부원 10명)
    if (promoTargets.club) {
      CLUB_MEMBERS.forEach(id => {
        if (USER_EMAILS[id]) {
          const name = STUDENT_LIST[id] || "학생";
          recipientMap.set(id, { email: USER_EMAILS[id], name });
        }
      });
    }
      ...
      <div className="bg-gray-800 p-6 rounded-xl shadow-xl border border-blue-600 mb-8">
        ...
        <div className="flex flex-wrap gap-6 mb-6">
          ...
          <label className="flex items-center gap-2 cursor-pointer border-l-2 border-gray-600 pl-6 ml-2">
            <input type="checkbox" checked={promoTargets.club} onChange={e => setPromoTargets({...promoTargets, club: e.target.checked})} className="w-5 h-5 accent-purple-600" /> <span className="text-purple-400 font-bold">테스트용 (동아리부원 10명)</span>
          </label>
        </div>
