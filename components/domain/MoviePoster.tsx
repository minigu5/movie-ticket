  ...
  return (
    <section className="w-full max-w-3xl mx-auto rounded-[var(--radius-lg)] border border-[var(--color-border-subtle)] bg-[var(--color-bg-elevated)] shadow-[var(--shadow-elev-1)] overflow-hidden">
      <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-0">
        <div className="relative aspect-[2/3] md:aspect-auto bg-[var(--color-bg-base)] border-b md:border-b-0 md:border-r border-[var(--color-border-subtle)]">
        ...
        <div className="p-6 md:p-7 flex flex-col gap-3">
          ...
          <div className="flex flex-wrap items-baseline gap-2">
            <h2 className="text-[22px] md:text-[26px] font-semibold tracking-tight text-[var(--color-text-primary)] text-balance">
              {movie.title}
            </h2>
            <Badge tone="muted">{movie.age_rating || "전체관람가"}</Badge>
          </div>
          <div className="flex flex-col gap-1.5 text-[13.5px] text-[var(--color-text-secondary)]">
